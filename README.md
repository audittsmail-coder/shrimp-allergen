...
- **`ponds/{pondId}`** (มีอยู่แล้ว) — เอกสาร 1 อันต่อ 1 บ่อ มีฟิลด์เดิม `name` (เลขบ่อ 3 หลัก
  ...
  - `pathogenAmmonia`, `pathogenNitrite` — ค่าแอมโมเนีย/ไนไตรท์ (mg/L) ของผลล่าสุด ถ้ามี
    (parser จะพยายามดึงจากข้อความสถานะ/แจ้งเตือนในรายงานให้อัตโนมัติ ถ้าไม่เจอจะเว้นว่างไว้
    ให้กรอกเองในตารางพรีวิวก่อนบันทึก)
  - `pathogenHistory[]` — array เก็บผลทุกสัปดาห์ที่เคยนำเข้า แต่ละรายการมี
    `{ weekDate, status, severity, worsened, ammonia, nitrite, reportId }` ใช้แสดงเทรนด์/ประวัติแบบเต็ม
...
3. กด **🔍 แยกข้อมูล** — ระบบจะอ่านตาราง "สถานะแต่ละบ่อ" และดึงชื่อฟาร์ม/บ่อ/สถานะ/ระดับ
   (ปกติ-เฝ้าระวัง-วิกฤต) ออกมาเป็นตารางที่แก้ไขได้ พร้อมพยายามดึงค่าแอมโมเนีย/ไนไตรท์ให้ถ้ามีในรายงาน
4. ตรวจสอบ/แก้ไขวันที่ สถานะ หรือเพิ่ม-ลบแถวได้ตามต้องการ — กรอกแอมโมเนีย/ไนไตรท์เองได้ถ้า parser หาไม่เจอ ก่อนบันทึก
