...
ไม่มี UI สำหรับตั้งค่า Firebase เอง — ถ้าต้องการใช้โปรเจกต์อื่นแทน ต้องแก้ `DEFAULT_CONFIG`
ใน `firebase.js` โดยตรง
...
- **`ponds/{pondId}`** (มีอยู่แล้ว) — เอกสาร 1 อันต่อ 1 บ่อ มีฟิลด์เดิม `name` (เลขบ่อ 3 หลัก
  ...
  - `pathogenAmmonia`, `pathogenNitrite`, `pathogenPh`, `pathogenAlkalinity`, `pathogenSalinity`
    — ค่าแอมโมเนีย/ไนไตรท์ (mg/L), pH, ด่าง (mg/L), ความเค็ม (ppt) ของผลล่าสุด ถ้ามี
    (parser จะพยายามดึงจากตารางคุณภาพน้ำ/พารามิเตอร์รายบ่อ/ข้อความสถานะในรายงานให้อัตโนมัติ
    ถ้าไม่เจอจะเว้นว่างไว้ให้กรอกเองในตารางพรีวิวก่อนบันทึก)
  - `pathogenHistory[]` — array เก็บผลทุกสัปดาห์ที่เคยนำเข้า แต่ละรายการมี
    `{ weekDate, status, severity, worsened, ammonia, nitrite, ph, alkalinity, salinity, reportId }`
    ใช้แสดงเทรนด์/ประวัติแบบเต็ม
...
3. กด **🔍 แยกข้อมูล** — ระบบจะอ่านตาราง "สถานะแต่ละบ่อ" และดึงชื่อฟาร์ม/บ่อ/สถานะ/ระดับ
   (ปกติ-เฝ้าระวัง-วิกฤต) ออกมาเป็นตารางที่แก้ไขได้ พร้อมพยายามดึงค่าแอมโมเนีย/ไนไตรท์/pH/ด่าง/ความเค็ม
   ให้ถ้ามีในรายงาน
4. ตรวจสอบ/แก้ไขวันที่ สถานะ หรือเพิ่ม-ลบแถวได้ตามต้องการ — กรอกค่าคุณภาพน้ำเองได้ถ้า parser หาไม่เจอ ก่อนบันทึก
